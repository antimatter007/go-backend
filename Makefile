...
createdb:
	docker exec -it postgres createdb --username=postgres --owner=postgres go_backend

dropdb:
	docker exec -it postgres dropdb go_backend
...
migratedown1:DB_URL=postgres://postgres:admin@localhost:5432/go_backend?sslmode=disable

network:
	docker network create bank-network

postgres:
	docker run --name postgres --network bank-network -p 5432:5432 -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=admin -d postgres:14-alpine
